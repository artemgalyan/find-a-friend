...
public class AdvertDao implements Dao<Integer, Advert> {
    private static final String SQL_SELECT_ALL_ADVERTS = "SELECT advert_id, advert_type, title, description, creation_date," +
            ...
            " FROM advert LEFT JOIN user USING(user_id) LEFT JOIN USING(role_id) LEFT JOIN USING(place_id)";
    private static final String SQL_SELECT_BY_ID = "SELECT advert_id, advert_type, title, description, creation_date," +
            " place.place_id, place.country, place.region, place.city, place.district," +
            " user.user_id, user.name, user.surname, user.email, user.phone_number, user.login, user.password, role.role_id, role.name" +
            " FROM advert LEFT JOIN user USING(user_id) LEFT JOIN USING(role_id) LEFT JOIN USING(place_id) WHERE advert_id=?";
    private static final String SQL_INSERT_ADVERT = "INSERT INTO advert VALUES(?,?,?,?, place_id, user_id)" +
    ...
    public static final String SQL_DELETE_BY_ID = "DELETE FROM advert WHERE advert_id=?";
