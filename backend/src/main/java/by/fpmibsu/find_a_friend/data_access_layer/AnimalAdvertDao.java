...
public class AnimalAdvertDao implements Dao<Integer, AnimalAdvert> {
    private static final String SQL_SELECT_ALL_ANIMALADVERTS = "SELECT animal_advert_id, title, description, creation_date, birthday, sex, castrated," +
            ...
            " FROM AnimalAdvert LEFT JOIN user USING(user_id) LEFT JOIN USING(role_id) LEFT JOIN USING(place_id) LEFT JOIN USING(animal_type_id)";
    private static final String SQL_SELECT_BY_ID = "SELECT animal_advert_id, title, description, creation_date, birthday, sex, castrated," +
            " AnimalType.animal_type_id, AnimalType.name," +
            " user.user_id, user.name, user.surname, user.email, user.phone_number, user.login, user.password, role.role_id, role.name," +
            "place.place_id, place.country, place.region, place.city, place.district"+
            " FROM AnimalAdvert LEFT JOIN user USING(user_id) LEFT JOIN USING(role_id) LEFT JOIN USING(place_id) LEFT JOIN USING(animal_type_id)" +
            "WHERE animal_advert_id=?";
    private static final String SQL_INSERT_ANIMALADVERT = "INSERT INTO advert VALUES(?,?,?,?,?,?, animal_type_id, user_id, place_id)" +
            ...
            " AND animal_type_id=?) ";
    private static final String SQL_DELETE_ANIMALADVERT = "DELETE FROM AnimalAdvert WHERE animal_advert_id=?";
    public static final String SQL_DELETE_BY_ID = "DELETE FROM AnimalAdvert WHERE animal_advert_id=?";
